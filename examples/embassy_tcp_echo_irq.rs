...
//! IRQ-driven variant of `embassy_tcp_echo`: identical TCP echo behaviour but
//! the embassy-time driver is the SYSTIMER+CLIC IRQ implementation in
//! `time_driver_irq.rs`. No `time_polling_task` is spawned; the executor
//! drops to `wfi` between SYSTIMER alarms and EMAC `wake_by_ref` cycles.
//!
//! Same TCP echo smoke: same EMAC + embassy-net stack as embassy_static_ping,
//! but with a TCP listener on :7777 that echoes any bytes back. Run from host:
//!
...
async fn dma_recovery_task() -> ! {
    // 50 ms (instead of 1 ms) is safe under `p4-time-driver-irq`:
    // RBU never fires in practice and a 50 ms recovery window is well below
    // any TCP-level retransmit timer. Saves ~2.5 % CPU in idle.
    loop {
...
fn main() -> ! {
    let mut uart = Uart0;
    let _ = writeln!(uart, "\n=== embassy_tcp_echo_irq alive ===");

    // Configure SYSTIMER alarm + CLIC routing + mtvec BEFORE any await.
...
fn init(spawner: Spawner) {
    ...
    // No time_polling_task — IRQ driver wakes the executor on its own.
